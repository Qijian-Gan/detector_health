...
for t=1:size(fileNameByYearMonth,1) 
    ...
    while (i<=numFileData)
        ...
        parfor j=i:min(i+numOfFile-1,numFileData)
            if(isempty(fileReadData) || ~any(strcmp(fileListData(j,:),fileReadData))) % Empty or Not yet read
                ...
                [tmpDevInv,tmpDevData,tmpSigInv,tmpSigData,tmpPlanPhase,tmpLastCyclePhase]=...
                    dp.parse_txt_detector(fileListData{j,:});
        ...
        if(~isempty(DevInv))
            ...
            dp.save_data_by_year_month(DevInv,'DevInv',strYearMonth)
        end
        
        % Save device data
        if(~isempty(DevData))
            disp('Saving/Updating device data!')
            dp.save_data_by_year_month(DevData,'DevData',strYearMonth)
        end
        
        % Save signal inventory
        if(~isempty(SigInv))
            disp('Saving/Updating intersection signal inventory!')
            dp.save_data_by_year_month(SigInv,'SigInv',strYearMonth)
        end
        
        % Save signal data
        if(~isempty(SigData))
            disp('Saving/Updating intersection signal data!')
            dp.save_data_by_year_month(SigData,'SigData',strYearMonth)
        end
        
        % Save planned phases
        if(~isempty(PlanPhase))
            disp('Saving/Updating signal planned phase!')
            dp.save_data_by_year_month(PlanPhase,'PlanPhase',strYearMonth)
        end
        
        % Save last-cycle phases
        if(~isempty(LastCyclePhase))
            disp('Saving/Updating signal last-cycle phase!')
            dp.save_data_by_year_month(LastCyclePhase,'LastCyclePhase',strYearMonth)
        end
        toc
